...
import { reportError, reportWarning } from '../../logging/report.ts';
...
export const handler = awslambda.streamifyResponse(
  async (
    ...
    if (!parsedHeader.success) {
      reportWarning('Request headers failed schema validation', {
    ...
    try {
      ...
      reportWarning('Failed to parse request body as JSON', { error });
    ...
    if (!parseResult.success) {
      reportWarning('Request body failed schema validation', {
    ...
    try {
      ...
      reportError('Agent runtime invocation failed', {
        error,
        threadId: body.threadId,
        runId,
      });
    ...
    if (!response.response) {
      reportError('Agent runtime returned no response body', {
        threadId: body.threadId,
        runId,
      });
      return streamedJsonErrorResponse(responseStream, 500, {
